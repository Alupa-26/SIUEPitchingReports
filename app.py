...
def COUGSPLUS_color_gradient(val):
...
def calculate_pitch_metrics_post_outing(data):
    ...
    for pitch_type, group in pitch_groups:
        ...
        metrics.append({
            ...
            "Pitches": total_pitches,
            "COUGSPLUS": round(group['COUGSPLUS'].mean(), 1)
        })
...
def calculate_pitch_metrics_bullpen(data):
    ...
    for pitch_type, group in pitch_groups:
        ...
        COUGSPLUS = group['COUGSPLUS'].mean()
        COUGSPLUS_value = "NA" if pd.isna(COUGSPLUS) else round(COUGSPLUS, 1)
...
st.set_page_config(page_title="SIUE Analytics", page_icon="⚾", layout="wide")
...
if uploaded_file is not None:
    ...
    if 'COUGSPLUS' in data.columns:
        data['COUGSPLUS'] = data['COUGSPLUS']
    else:
        data['COUGSPLUS'] = None  
        ...
        if report_type == "Post-Outing Report":
            for pitcher in unique_pitchers:
                ...
                def apply_row_colors(row):
                    ...
                    return [f'background-color: {color}' if col != 'COUGSPLUS' else '' for col in df_metrics.columns]

                df_metrics_styled = df_metrics.style.map(
                    COUGSPLUS_color_gradient, subset=['COUGSPLUS']
                ).apply(
